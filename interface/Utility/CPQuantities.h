...
public:
	...
	double CalculatePhiStarCPPolVecComb(RMFLV charged1, RMFLV charged2, RMFLV::BetaVector h1, RMFLV ref2, TVector3 ref2TV3, bool firstNegative);
	...
	ROOT::Math::SMatrix<float,3,3, ROOT::Math::MatRepStd< float, 3, 3 >> CalculatePCACovariance(double B, short charge, std::vector<float> h_param, ROOT::Math::SMatrix<float,5,5, ROOT::Math::MatRepSym<float,5>> cov, RMPoint ref, RMPoint PrV, SMatrixSym3D SigmaPrV, double xBest, KLepton* recoParticle);
