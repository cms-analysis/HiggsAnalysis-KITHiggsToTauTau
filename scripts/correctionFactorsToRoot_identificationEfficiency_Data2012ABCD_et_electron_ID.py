...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description="Store correction factors in ROOT histograms. Here: identification efficiencies (ET channel, electron ID, Data2012ABCD)",
	                                 parents=[logger.loggingParser])

	parser.add_argument("-n", "--histogram-name", default="identificationEfficiency",
	                    help="Histogram name. [Default: %(default)s]")
	parser.add_argument("-o", "--output",
	                    default="$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/root/identificationWeights/identificationEfficiency_Data2012ABCD_et_electron_ID.root",
	...
	histogram.SetBinContent(histogram.FindBin(24.0, 0.0), 0.7380)
	histogram.SetBinContent(histogram.FindBin(24.0, 1.479), 0.3319)
	histogram.SetBinContent(histogram.FindBin(30.0, 0.0), 0.8500)
	histogram.SetBinContent(histogram.FindBin(30.0, 1.479), 0.5342)
	
	histogram.SetBinError(histogram.FindBin(24.0, 0.0), 0.0010)
	histogram.SetBinError(histogram.FindBin(24.0, 1.479), 0.0017)
