...
if __name__ == "__main__":
	...
	parser.add_argument("-x", "--quantities", default="0", nargs="+",
	...
	parser.add_argument("--interpret-as-BDT", action="store_true", default=False, help="Interpret quantities that are not found in the binnings dictionary as BDT scores with values from -1 to 1. This is done anyway for quantities with 'BDT' in their name. [Default:%(default)s]")
	...
	parser.add_argument("--n-bins", type=float, default=100.0,
						help="Total number of bins in multidimensional histogram. [Default: %(default)s]")
			...
			for shape_systematic, list_of_samples in datacards_per_channel_category.get_samples_per_shape_systematic().iteritems():
				...
				for shift_up in ([True] if nominal else [True, False]):
					...
					if len(args.quantities)>1:
						#Reorder bins of given final discriminants (multi-dimensional histogram) to an one-dimensional pseudo score which then works as the final discriminant.
						#The pseudo score is created like a numeral system where each digit represents a certain input quantity and its value corresponds to the bin number. A binning of nbins1D = 10 would result in a decimal system.
						qintervals = [] #intervals covered by the quantities
						subscores = [] #digit*digitweight e.g. [1000, 200, 50, 0] for 1250 in a decimal system
						for i, q in enumerate(args.quantities):
							#find intervals to be scaled on [0, nbins1D]
							binnings_key = "binningMVAStudies_"+category+"_%s"%q
							if binnings_key in binnings_settings.binnings_dict:
								binnings = binnings_settings.binnings_dict[binnings_key].split(" ")
								qintervals.append([float(binnings[0]), float(binnings[len(binnings)-1])])
							else:
								if ("BDT" in q) or args.interpret_as_BDT:
									#non declared input quantities with "BDT" in name are assumed to be BDT scores
									warnings.warn("Warning: No binning found for %s in dictionary. It is assumed to be a BDT score with values from -1 to 1."%q)
									qintervals.append([-1.0, 1.0])
								else:
									raise AttributeError("No binning found for %s in dictionary!"%q)
							##if args.two_binning:
							##	nbins1D=2
							##subscores.append("(int({0}>=0.6)*{1})".format(q, float(int(nbins1D**i))))
							##else:
							#calculate (digit corresponding to q)*(digit weight) i.e. int(...)*{4}
							subscores.append("(int(({0}-({1}))*{3}/{2})*{4})".format(q, float(qintervals[i][0]), float(qintervals[i][1])-float(qintervals[i][0]), float(nbins1D), float(int(nbins1D**i))))
						#merge digits to pseudo score and book corresponding binning
						config["x_expressions"] = "+".join(subscores)
						log.debug(config["x_expressions"])
						config["x_bins"] = [" ".join([str(float(f)) for f in range(int(nbins1D ** len(args.quantities))+1)])]
						
						
					else:
						config["x_expressions"] = [args.quantities[0]]
						binnings_key = "binningHtt13TeV_"+category+"_%s"%args.quantities
	...
	#if args.quantities[0] == "m_sv" and not(args.do_not_normalize_by_bin_width):
	#	args.args += " --y-label 'dN / dm_{#tau #tau}  (1 / GeV)'"

	datacards.prefit_postfit_plots(datacards_cbs, datacards_postfit_shapes, plotting_args={"ratio" : args.ratio, "args" : args.args, "lumi" : args.lumi, "x_expressions" : ";".join(args.quantities), "normalize" : not(args.do_not_normalize_by_bin_width), "era" : args.era}, n_processes=args.n_processes)
