...
if __name__ == "__main__":
	...
	sample_settings = samples.Samples(embedding=args.emb,embedding_weight="(1.0)",ttbar_retuned=False,legacy=args.legacy)
	...
	# 	)
	if args.emb:
		command = "MorphingSMCPDecays18 --era={ERA} --channels={CHANNELS} --do_jetfakes=true --do_embedding=true --postfix=\"-2D\" --ttbar_fit=false {INIT} {SHAPE_UNCS}".format(
			ERA=args.era,
			CHANNELS = channelstring,
			INIT="--only_init="+os.path.join(init_directory, "init"),
		...
		command = "MorphingSMCPDecays18 --era={ERA} --channels={CHANNELS} --do_jetfakes=true --do_embedding=false --postfix=\"-2D\" --ttbar_fit=false {INIT} {SHAPE_UNCS}".format(
			ERA=args.era,
			CHANNELS = channelstring,
			INIT="--only_init="+os.path.join(init_directory, "init"),
			SHAPE_UNCS="--no_shape_systs=1" if args.no_shape_uncs else "",
	...
	for chan in channel_list:
		if chan == "tt":
			category_replacements["_".join([chan,args.era,"jetFakes"])] 					= "_".join([chan,"jetFakes"])
			category_replacements["_".join([chan,args.era,"zttEmbed"])] 					= "_".join([chan,"zttEmbed"])
			category_replacements["_".join([chan,args.era,"higgs_Pi_Pi"])] 					= "_".join([chan,"higgs_Pi_Pi"])
			# category_replacements["_".join([chan,args.era,"higgs_Pi_Pi"])] 					= "_".join([chan,"higgs_Pi_Pi_IPSig"])
			category_replacements["_".join([chan,args.era,"higgs_Pi_Rho_Mixed"])] 			= "_".join([chan,"higgs_Pi_Rho_Mixed"])
			# category_replacements["_".join([chan,args.era,"higgs_Pi_Rho_Mixed"])] 			= "_".join([chan,"higgs_Pi_Rho_Mixed_IPSig"])
			category_replacements["_".join([chan,args.era,"higgs_Pi_0A1_Mixed"])] 			= "_".join([chan,"higgs_Pi_0A1_Mixed"])
			# category_replacements["_".join([chan,args.era,"higgs_Pi_0A1_Mixed"])] 			= "_".join([chan,"higgs_Pi_0A1_Mixed_IPSig"])
			category_replacements["_".join([chan,args.era,"higgs_Pi_A1_Mixed"])] 			= "_".join([chan,"higgs_Pi_A1_Mixed"])
			# category_replacements["_".join([chan,args.era,"higgs_Pi_A1_Mixed"])] 			= "_".join([chan,"higgs_Pi_A1_Mixed_IPSig"])
			category_replacements["_".join([chan,args.era,"higgs_Rho_Rho"])] 				= "_".join([chan,"higgs_Rho_Rho"])
			category_replacements["_".join([chan,args.era,"higgs_0A1_Rho_and_0A1_0A1"])] 	= "_".join([chan,"higgs_0A1_Rho_and_0A1_0A1"])
			category_replacements["_".join([chan,args.era,"higgs_A1_Rho"])] 				= "_".join([chan,"higgs_A1_Rho"])
			category_replacements["_".join([chan,args.era,"higgs_A1_0A1"])] 				= "_".join([chan,"higgs_A1_0A1"])
			category_replacements["_".join([chan,args.era,"higgs_A1_A1"])] 					= "_".join([chan,"higgs_A1_A1"])
			category_replacements["_".join([chan,args.era,"higgs_A1_A1_PolVec"])] 			= "_".join([chan,"higgs_A1_A1_PolVec"])
			category_replacements["_".join([chan,args.era,"higgs_A1_A1_SimpleFitPolVec"])] 	= "_".join([chan,"higgs_A1_A1_SimpleFitPolVec"])

	# initialise datacards
	# year_string = "/2017/" if args.era=="2017" else ""
	year_string = "/"+args.era+"/" if args.era else ""
		...
		for official_category in categories:
			...
			for shape_systematic, list_of_samples in datacards_per_channel_category.get_samples_per_shape_systematic().iteritems():
				...
				for shift_up in ([True] if nominal else [True, False]):
					...
					# asimov_nicks = list(list_of_samples)
					# print asimov_nicks
					# if "data" in asimov_nicks:
					# 	asimov_nicks.remove("data")
					# print asimov_nicks
					# for index, nick in enumerate(asimov_nicks):
					# 	if "zh" in nick or "wh" in nick or "qqh" in nick or "ggh" in nick:
					# 		if "mm" in nick or "ps" in nick:
					# 			print nick
					# 			asimov_nicks.remove(nick)
					# 		else:
					# 			asimov_nicks[index] = nick + "125"
					#
					# print asimov_nicks
					# asimov_nicks = ['ztt_emb', 'ttt', 'vvt', 'whsm125', 'wfakes', 'zhsm125', 'zl', 'gghsm125', 'ff', 'qqhsm125']
					asimov_nicks = ['ztt_emb', 'ttt', 'vvt', 'whps125', 'wfakes', 'zhps125', 'zl', 'gghps125', 'ff', 'qqhps125']

					config = sample_settings.get_config(
							samples=[getattr(samples.Samples, sample) for sample in list_of_samples],
							channel=channel if category not in "em_ttbar" else "em",
							# category="catcptautau2017_"+category+category_suffix,
							category="catCPFinalstate13TeV_"+category+category_suffix,
							# category=args.category_prefix+category+category_suffix,
							weight=args.weight,
							lumi = args.lumi * 1000,
							exclude_cuts=exclude_cuts,
							higgs_masses=higgs_masses,
							cut_type="cpggh2016" if args.era == "2016" else "cptautau2017legacy" if (args.era == "2017" or args.era == "2018") else "baseline",
							...
							cpfinalstate = True,
							state = "finalState",
							# asimov_nicks = ["ff", "zl", "vvt", "ttt", "gghsm125", "qqhsm125", "whsm125", "zhsm125", ("ztt_emb" if args.emb else "ztt")] if args.use_asimov_dataset else [], # TODO: generalize
					...
					config["x_expressions"] = ["recoPhiStarCPCombMergedHelrPVBS"]

					binnings_key = "tt_" + args.quantity;

					# Use 2d plots for 2d categories
					# CP Finalstate analysis
					if "jetFakes" in category or "zttEmbed" in category:
						config["x_expressions"] = ["IC_BDT_max_score"]
						config["x_bins"] = [binnings_settings.binnings_dict["binningHtt13TeV_CPFinalState_"+channel+"_bkg_MVA_score"]]
					elif "higgs" in category:
						y_expression = "recoPhiStarCPCombMergedHelrPVBS"
						if "0A1_Rho_and_0A1_0A1" in category or "Rho_Rho" in category:
							y_expression = "recoPhiStarCPRhoMerged"
						elif "Pi_Pi" in category:
							y_expression = "recoPhiStarCPrPVBS"
						elif "Pi_A1_mixed" in category:
							y_expression = "recoPhiStarCPPolVecCombOneProngTauA1HelrPVBS"
						config["y_expressions"] = [y_expression]
						config["y_bins"] = [binnings_settings.binnings_dict["binningHtt13TeV_"+category+"_PhiStarCP"]]
						if channel == "tt":
							config["x_expressions"] = ["IC_BDT_max_score"]
							config["x_bins"] = [binnings_settings.binnings_dict["binningHtt13TeV_CPFinalState_"+channel+"_higgs_MVA_score"]]
					...
					for sample in list_of_samples:
						...
						if sample in ["gghmm", "gghps", "gghsm", "qqhmm", "qqhps", "qqhsm", "whmm", "whps", "whsm", "zhmm", "zhps", "zhsm"]:
	...
	if args.n_plots[0] != 0 and "t2w" in args.steps:
		...
		for channel in channel_list:
			print "hadd_shapes step for channel:", channel
			if channel != "ttbar":
				channel_ = channel
			else: channel_="em"
	...
	if "datacards" in args.steps:
		print "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
		print "datacards step for channel:", channelstring
		if args.emb:
			datacards_module._call_command([
				"MorphingSMCPDecays18 --output_folder={OUTPUT_SUFFIX} --era={ERA} --channels={CHANNELS} --do_jetfakes=true --do_embedding=true --postfix=\"-2D\" {SHAPE_UNCS} {SCALE_SIG} --ttbar_fit=false --input_folder_mt={OUTPUT_SUFFIX}/mt/ --input_folder_et={OUTPUT_SUFFIX}/et/ --input_folder_tt={OUTPUT_SUFFIX}/tt/".format(
			...
			datacards_module._call_command([
				"MorphingSMCPDecays18 --output_folder={OUTPUT_SUFFIX} --era={ERA} --channels={CHANNELS} --do_jetfakes=true --do_embedding=false --postfix=\"-2D\" {SHAPE_UNCS} {SCALE_SIG} --ttbar_fit=false --input_folder_mt={OUTPUT_SUFFIX}/mt/ --input_folder_et={OUTPUT_SUFFIX}/et/ --input_folder_tt={OUTPUT_SUFFIX}/tt/".format(
				OUTPUT_SUFFIX=args.output_suffix,
				ERA=args.era,
				CHANNELS = channelstring,
				SHAPE_UNCS="--no_shape_systs=1" if args.no_shape_uncs else "",
	...
	if "t2w" in args.steps:
		print "t2w step for channel:", channelstring
		datacards_module._call_command([
	...
	if "likelihoodscan" in args.steps:
		...
		datacards_module._call_command([
				# "combineTool.py -m 125 -M MultiDimFit --setParameters muV=1,alpha=0,muggH=1,mutautau=1,lumi_scale=1 --setParameterRanges alpha=-90,90 --points 21 --redefineSignalPOIs alpha -d output/{OUTPUT_SUFFIX}/{{cmb,{CHANNELS}}}/125/ws.root --algo grid -t -1 --there -n .alpha --alignEdges 1 --cminDefaultMinimizerStrategy=0".format(
				"combineTool.py -m 125 -M MultiDimFit --setParameters muV=1,alpha=90,muggH=1,mutautau=1,lumi_scale=1 --setParameterRanges alpha=-90,90 --points 21 --redefineSignalPOIs alpha -d output/{OUTPUT_SUFFIX}/{{cmb,{CHANNELS}}}/125/ws.root --algo grid -t -1 --there -n .alpha --alignEdges 1 --cminDefaultMinimizerStrategy=0".format(
				# "combineTool.py -m 125 -M MultiDimFit --setParameters muV=1,alpha=0,muggH=1,mutautau=1,lumi_scale=2.3 --setParameterRanges alpha=-90,90 --points 21 --redefineSignalPOIs alpha -d output/{OUTPUT_SUFFIX}/{{cmb,{CHANNELS}}}/125/ws.root --algo grid -t -1 --there -n .alpha --alignEdges 1 --cminDefaultMinimizerStrategy=0".format(
