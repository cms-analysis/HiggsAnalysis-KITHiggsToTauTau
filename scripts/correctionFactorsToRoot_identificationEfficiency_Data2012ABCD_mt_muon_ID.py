...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description="Store correction factors in ROOT histograms. Here: identification efficiencies (MT channel, muon ID, Data2012ABCD)",
	                                 parents=[logger.loggingParser])

	parser.add_argument("-n", "--histogram-name", default="identificationEfficiency",
	                    help="Histogram name. [Default: %(default)s]")
	parser.add_argument("-o", "--output",
	                    default="$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/root/identificationWeights/identificationEfficiency_Data2012ABCD_mt_muon_ID.root",
	...
	histogram.SetBinContent(histogram.FindBin(20.0, 0.0), 0.9471)
	histogram.SetBinContent(histogram.FindBin(20.0, 0.8), 0.9422)
	histogram.SetBinContent(histogram.FindBin(20.0, 1.2), 0.9387)
	histogram.SetBinContent(histogram.FindBin(30.0, 0.0), 0.9488)
	histogram.SetBinContent(histogram.FindBin(30.0, 0.8), 0.9434)
	histogram.SetBinContent(histogram.FindBin(30.0, 1.2), 0.9380)
	
	histogram.SetBinError(histogram.FindBin(20.0, 0.0), 0.0003)
	histogram.SetBinError(histogram.FindBin(20.0, 0.8), 0.0007)
