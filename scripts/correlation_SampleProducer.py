...
def calculate_partial_correlation(config):
	...
	root_file_name_list = []
	stored_files_list.append("%s_%s_%s.root"%(config["storage_name_extension"], config["channel"], config["request_nick"]))
	config["storage_file"] = stored_files_list[-1]
	config["storage_ntuple"]=config["folders"][0].replace("/ntuple", "")
	if config["prepare_samples"]:
		cuts = ""
	...
	nick_path = os.path.join(config["dir_path"], config["request_nick"])
...
if __name__ == "__main__":
	...
	parser.add_argument("-w", "--weight", default="1.0",
						...
						[Default: %(default)s]""")
	parser.add_argument("-P", "--prepare-samples", action="store_false", default=True,
						help="specify if you already produced reduced files and just want to rerun with different variables. [Default: %(default)s]")
	...
	for channel in args.channels:
		...
		for requested_sample in args.samples:
			...
			config["prepare_samples"] = args.prepare_samples
			config["input_dir"] = args.input_dir
			config["storage_name_extension"] = storage_name_extension
