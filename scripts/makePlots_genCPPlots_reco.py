...
if __name__ == "__main__":
	...
	w_dy1=["((OneProngChargedPart1PdgId == -11 && OneProngChargedPart2PdgId == 13) && (Tau1OneProngsSize == 3 && Tau2OneProngsSize == 3) && (TauMProngEnergy >= 0.44 && TauPProngEnergy >= 0.44))"]
	w_dy4=["((OneProngChargedPart1PdgId != -999 && OneProngChargedPart1PdgId != 11 && OneProngChargedPart1PdgId != 13 && OneProngChargedPart2PdgId != -999 && OneProngChargedPart2PdgId != 11 && OneProngChargedPart2PdgId != 13) && (Tau1OneProngsSize == 2 && Tau2OneProngsSize == 2) && (TauMProngEnergy >= 0.55 && TauPProngEnergy >= 0.55))"]
	w_dy3=["(abs(OneProngChargedPart1PdgId) != 999 && abs(OneProngChargedPart1PdgId) != 11 && abs(OneProngChargedPart1PdgId) != 13 && abs(OneProngChargedPart2PdgId) == 13 && (Tau1OneProngsSize == 2 && Tau2OneProngsSize == 3) && (TauMProngEnergy >= 0.44 && TauPProngEnergy >= 0.44))"]
	w_dy2=["(abs(OneProngChargedPart1PdgId) != 999 && abs(OneProngChargedPart1PdgId) != 11 && abs(OneProngChargedPart1PdgId) != 13 && abs(OneProngChargedPart2PdgId) == 11 && (Tau1OneProngsSize == 2 && Tau2OneProngsSize == 3) && (TauMProngEnergy >= 0.44 && TauPProngEnergy >= 0.44))"]
	...
	w_evenandodd=[[w_em[0],w_dy1,w_dy1],[w_et[0],w_dy2,w_dy2],[w_mt[0],w_dy3,w_dy3],[w_tt[0],w_dy4,w_dy4]]
	

	w=[w_em,w_et,w_mt,w_tt]
	names=["_em","_et","_mt","_tt"]
	...
	plot_configs = []
	f=["em_jecUncNom/ntuple", "et_jecUncNom_tauEsNom/ntuple", "mt_jecUncNom_tauEsNom/ntuple", "tt_jecUncNom_tauEsNom/ntuple"]
        f1=["et_jecUncNom_tauEsNom/ntuple"]
	#all mixingangles in 1 plot
	for i in xrange(4):		
		config1 = jsonTools.JsonDict(os.path.expandvars("$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/plots/configs/gen_CP/phiCP_onlyleptonic.json"))
		#all mixingangles in 1 plot
		config1["folders"]= [f[i]]
		...
		config1["files"]=["GluGluHToTauTauM*/*.root VBFH*/*.root W*HToTauTauM*/*.root ZH*/*.root"]
		plot_configs.append(config1)	
	#only CP-even and CP-odd in 1 plot with DYbkg and SUSY
	for j in xrange(4):	
		config2=jsonTools.JsonDict(os.path.expandvars("$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/plots/configs/gen_CP/phiCPevenandodd_onlyleptonic.json"))		
		config2["folders"]= [f[j]]	
		...
		config2["files"]=["GluGluHToTauTauM*/*.root VBFH*/*.root W*HToTauTauM*/*.root ZH*/*.root","SUSY*/*.root","DYJets*/*root"]
