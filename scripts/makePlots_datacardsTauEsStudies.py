...
if __name__ == "__main__":
	...
	# Initialise datacards for combine
	...
	if len(args.channels) > 1: datacard_filename_templates.append("datacards/decaymode/${BINID}/${ANALYSIS}_${BINID}_${ERA}.txt")
	output_root_filename_template = "datacards/common/${ANALYSIS}.input_${ERA}.root"

	# Build categories and bin id's for CombineHarvester
	# {str('channel'): {str('cattegory'): int('bin_id')}}
	# bin_id is not unique accross channels
	categories = []
	mapping_category2binid = {}
	for channel in args.channels:
		mapping_category2binid.setdefault(channel, {})

		if quantity == "m_2": bin_id = 2000
		else:                 bin_id = 3000
		if args.eta_binning:  bin_id = bin_id + 2000

		for decayMode in args.decay_modes:
			bin_id += 1

			for weight_index in range(0, len(weight_bins)):
				# for weight_index in weight_bins:
				bin_id += 1
	...
	for category in categories:
		...
		useRelaxedIsolation = weight_index > 0
		...
		for shape_systematic, list_of_samples in datacards_per_channel_category.get_samples_per_shape_systematic().iteritems():
			...
			if args.background_method == "new":
				if  "qcd" in list_of_samples and "wj"  not in list_of_samples: list_of_samples += ["wj"]
	...
	# datacards.cb.FilterProcs(lambda proc: remove_procs_and_systs_with_zero_yield2(proc, datacards))
	...
	print "2nd combine call to get deltaNLL distribution"
