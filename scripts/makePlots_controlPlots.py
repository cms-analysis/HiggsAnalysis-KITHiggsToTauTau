		...
		for category in args.categories:
			for quantity in args.quantities:
				...
				if category is not None:
					binnings_key1 = category.replace("cat","binning")+"_"+quantity
					config["title"] = "channel_"+channel+"_"+"_".join(category.split("_")[2:])
				else:
					config["title"] = "channel_"+channel
					binnings_key1 = channel+"_"+quantity
				binnings_key2 = channel+"_"+quantity
				if binnings_key1 in binnings_settings.binnings_dict:
					config["x_bins"] = json_config.pop("x_bins", [binnings_key1])
				elif binnings_key2 in binnings_settings.binnings_dict:
					config["x_bins"] = json_config.pop("x_bins", [binnings_key2])
				...
				if args.cms:
					...
					config["legend"] = [0.7, 0.47, 0.95, 0.90] if args.ratio else [0.7, 0.47, 0.9, 0.82]
				elif args.shapes:
					config["legend"] = [0.55, 0.65, 0.9, 0.88]
				else:
					config["y_rel_lims"] = [0.5, 10.0] if "--y-log" in args.args else [0.0, 1.5 if args.ratio else 1.4]
					config["legend"] = [0.23, 0.70, 0.9, 0.90] if args.ratio else [0.23, 0.70, 0.9, 0.86]
