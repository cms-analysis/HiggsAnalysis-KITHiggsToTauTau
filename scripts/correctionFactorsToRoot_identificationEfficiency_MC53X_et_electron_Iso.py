...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description="Store correction factors in ROOT histograms. Here: identification efficiencies (ET channel, electron Iso, MC53X)",
	                                 parents=[logger.loggingParser])

	parser.add_argument("-n", "--histogram-name", default="identificationEfficiency",
	                    help="Histogram name. [Default: %(default)s]")
	parser.add_argument("-o", "--output",
	                    default="$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/root/identificationWeights/identificationEfficiency_MC53X_et_electron_Iso.root",
	...
	histogram.SetBinContent(histogram.FindBin(24.0, 0.0), 0.7779)
	histogram.SetBinContent(histogram.FindBin(24.0, 1.479), 0.8530)
	histogram.SetBinContent(histogram.FindBin(30.0, 0.0), 0.9068)
	histogram.SetBinContent(histogram.FindBin(30.0, 1.479), 0.9237)
	...
	histogram.SetBinError(histogram.FindBin(30.0, 1.479), 0.0004)
