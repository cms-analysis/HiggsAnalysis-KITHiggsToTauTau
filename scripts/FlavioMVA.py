...
##Define constants
version = ""
sample_dir = "/net/scratch_cms3b/{}/artus/AllSamples/merged/".format(os.environ["USER"])
...
output_result = "$CMSSW_BASE/src/FlavioOutput/BDT/outputB_{channel}.root"
...
def parser():
	...
	parser.add_argument("--results", action = "store_true", help = "Give .xml or .root output from TMVA to show results with the TMVA GUI")
...
def training(channelselection): ##NEU: Argument welchen channel man moechte
	...
	for channel in channelselection: 
		...
		for sample_type, cut_label in zip([sig_name, sig_name, bkg_name, bkg_name], ["A", "B", "A", "B"]):
			sub_config.append(input_file.format(sample_type = sample_type, cut_label = cut_label, channel = "em" if channel== "etm" or channel == "mte" else channel)) 
...
def do_training(args):
	...
	treechannel = "em" if channel == "etm" or channel == "mte" else channel

	##Get the input trees
	file_sig_train = ROOT.TFile(sig_train, "READ")
	tree_sig_train = ROOT.TTree()
	file_sig_train.GetObject(treechannel, tree_sig_train)

	file_sig_test = ROOT.TFile(sig_test, "READ")
	tree_sig_test = ROOT.TTree()
	file_sig_test.GetObject(treechannel, tree_sig_test)

	file_bkg_train = ROOT.TFile(bkg_train, "READ")
	tree_bkg_train = ROOT.TTree()
	file_bkg_train.GetObject(treechannel, tree_bkg_train)

	file_bkg_test = ROOT.TFile(bkg_test, "READ")
	tree_bkg_test = ROOT.TTree()
	file_bkg_test.GetObject(treechannel, tree_bkg_test)
	...
	for index, weight in enumerate(cuts.CutStringsDict().lfv(treechannel, "").values()[1:]):
		weights = weights + "*" if index != 0 else weights
		weights += weight

	##Set signal/background event weights
	factory.SetSignalWeightExpression("(jetCorrectionWeight)*(eventWeight)*(lheZto{channel} > 0.5)*".format(channel=channel[: -1].upper() if channel == "etm" or channel == "mte" else channel.upper() ) + weights)
	...
	sig_cut = ROOT.TCut("")
	bkg_cut = ROOT.TCut("")
	n_option =  ROOT.TString("") #ROOT.TString("nTrain_Signal=10000:nTest_Signal=10000:nTrain_Background=10000:nTest_Background=10000:SplitMode=Random",) 
...
def application(channelselection): 
	...
	if not local:
		...
		entries = [
				...
				[("executable", "kappa.sh")],
				[("parameters", "FILE_NAME CHANNEL"), ("FILE_NAME", " ".join(mc_list)), ("CHANNEL", "em")] 
		]
...
def show_results(channel):
	for chan in channel:
		ROOT.TMVA.TMVAGui(output_result.format(channel = chan))
...
def main():
	...
	if(args.create_input_trees):
		create_input_trees(args.channel) 

	if(args.training):	
		training(args.channel) 
	if(args.application):
		application(args.channel) 

	if(args.file_attach):
		attach(args.file_attach) 

	if(args.results):
		show_results(args.channel) 
