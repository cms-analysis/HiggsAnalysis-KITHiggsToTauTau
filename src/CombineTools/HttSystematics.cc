...
void HttSystematics::AddSystematicsEEMM(ch::CombineHarvester & cb) {
	ch::CombineHarvester src = cb.cp();
	src.channel({"ee", "mm"});
	...
	auto signal = ch::Set2Vec(src.cp().signals().process_set());
...
void HttSystematics::AddSystematicsEM(ch::CombineHarvester & cb) {
	ch::CombineHarvester src = cb.cp();
	src.channel({"em"});
...
void HttSystematics::AddSystematicsETMT(ch::CombineHarvester & cb) {
	ch::CombineHarvester src = cb.cp();
	src.channel({"et", "mt"});
	...
	src.cp().channel({"et"}).process({"QCD"}).bin_id({3})
	    .AddSyst(cb, "CMS_htt_QCDShape_et_1jet_medium_$ERA", "shape",
	      SystMap<>::init(1.00));

	src.cp().channel({"mt"}).process({"QCD"}).bin_id({4})
	    .AddSyst(cb, "CMS_htt_QCDShape_mt_1jet_high_lowhiggs_$ERA", "shape",
	      SystMap<>::init(1.00));

	src.cp().channel({"mt"}).process({"QCD"}).bin_id({6}).era({"7TeV"})
	    .AddSyst(cb, "CMS_htt_QCDShape_mt_vbf_$ERA", "shape",
	      SystMap<>::init(1.00));
	src.cp().channel({"et"}).process({"QCD"}).bin_id({6}).era({"7TeV"})
	    .AddSyst(cb, "CMS_htt_QCDShape_et_vbf_$ERA", "shape",
	      SystMap<>::init(1.00));

	src.cp().channel({"mt"}).process({"QCD"}).bin_id({6}).era({"8TeV"})
	    .AddSyst(cb, "CMS_htt_QCDShape_mt_vbf_loose_$ERA", "shape",
	      SystMap<>::init(1.00));
	...
	src.cp().channel({"mt"}).process({"ZL"})
	    .AddSyst(cb,
	    "CMS_htt_ZLScale_mt_$ERA", "shape", SystMap<>::init(1.00));
	src.cp().channel({"et"}).process({"ZL"})
	    .AddSyst(cb,
	    "CMS_htt_ZLScale_et_$ERA", "shape", SystMap<>::init(1.00));
}

void HttSystematics::AddSystematicsTT(ch::CombineHarvester & cb) {
	ch::CombineHarvester src = cb.cp();
	src.channel({"tt"});
