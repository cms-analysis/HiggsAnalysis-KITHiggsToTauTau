...
void DiJetQuantitiesProducer::Init(setting_type const& settings, metadata_type& metadata)
{
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetPt", [this](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return DiJetQuantitiesProducer::GetDiJetQuantity(product, [](RMDLV diJetSystem) -> double
	{
		return diJetSystem.Pt(); });
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetEta", [this](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return DiJetQuantitiesProducer::GetDiJetQuantity(product, [](RMDLV diJetSystem) -> double
	{
		return diJetSystem.Eta(); });
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetPhi", [this](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return DiJetQuantitiesProducer::GetDiJetQuantity(product, [](RMDLV diJetSystem) -> double
	{
		return diJetSystem.Phi(); });
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetMass", [this](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return DiJetQuantitiesProducer::GetDiJetQuantity(product, [](RMDLV diJetSystem) -> double
	{
	...
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "signedDiJetDeltaPhi", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return product.m_diJetSystemAvailable ? ROOT::Math::VectorUtil::DeltaPhi(product.m_validJets[0]->p4, product.m_validJets[1]->p4) * (product.m_validJets[0]->p4.Eta() > product.m_validJets[1]->p4.Eta() ? 1.0 : -1.0) :
		                                        DefaultValues::UndefinedDouble;
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetAbsDeltaEta", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return product.m_diJetSystemAvailable ? std::abs(product.m_validJets[0]->p4.Eta() - product.m_validJets[1]->p4.Eta()) :
		                                        -1;
	});
	LambdaNtupleConsumer<HttTypes>::AddFloatQuantity(metadata, "diJetdiLepPhi", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return product.m_diJetSystemAvailable ? (product.m_diJetSystem + product.m_diLeptonSystem).Phi() :
		                                        DefaultValues::UndefinedDouble;
	});
	LambdaNtupleConsumer<HttTypes>::AddBoolQuantity(metadata, "centralJet30Exists", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return (product.m_nCentralJets30 > 0 ? true : false);
	});
	LambdaNtupleConsumer<HttTypes>::AddIntQuantity(metadata, "nCentralJets20", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
		return product.m_nCentralJets20;
	});
	LambdaNtupleConsumer<HttTypes>::AddIntQuantity(metadata, "nCentralJets30", [](event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata) {
