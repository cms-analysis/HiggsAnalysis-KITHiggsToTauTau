...
// 			tau->p4 = tau->p4 * (1.012 + 0.001 * std::min(std::max(tau->p4.Pt() - 32.0, 0.0), 18.0));
	...
	else if  (tauEnergyCorrection == TauEnergyCorrection::LEGACY2016 || tauEnergyCorrection == TauEnergyCorrection::LEGACY2017 || tauEnergyCorrection == TauEnergyCorrection::LEGACY2018)
	{
		...
		int bin = TauEnergyCorrectionHist->GetXaxis()->FindBin(dm);
		...
		// LOG(INFO) << "tau->decayMode: " << tau->decayMode;
		// LOG(INFO) << "TauEnergyCorrectionShift: " << TauEnergyCorrectionShift;
		// LOG(INFO) << "TauEnergyCorrectionShift -1: " << TauEnergyCorrectionShift - 1.0;
		// LOG(INFO) << "Before tau->p4: " << tau->p4;
		tau->p4 = tau->p4 * TauEnergyCorrectionShift;
		// LOG(INFO) << "After tau->p4: " << tau->p4;
	}
	else if(tauEnergyCorrection == TauEnergyCorrection::EMBEDDING)
	{
		bool isShiftUp = static_cast<HttSettings const&>(settings).GetIsShiftUp();
		bool isNominal = static_cast<HttSettings const&>(settings).GetIsNominal();
		...
		{
			case 0:
				TauEnergyCorrectionShift = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProng();
				errUp = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProngShiftUp();
				errDown = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProngShiftDown();
				break;
			case 1: // fall-through
			case 2:
				TauEnergyCorrectionShift = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProngPiZeros();
				errUp = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProngPiZerosShiftUp();
				errDown = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionOneProngPiZerosShiftDown();
				break;
			case 10: // fall-through
			case 11:
				TauEnergyCorrectionShift = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionThreeProng();
				errUp = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionThreeProngShiftUp();
				errDown = static_cast<HttSettings const&>(settings).GetTauEnergyCorrectionThreeProngShiftDown();
				break;
			default:
		...
		if (!isNominal)
		{
			if (isShiftUp)
			{
				TauEnergyCorrectionShift += errUp;
			}
			else
			{
				TauEnergyCorrectionShift -= errDown;
			}
		}
		// LOG(INFO) << "tau->decayMode: " << tau->decayMode;
		// LOG(INFO) << "TauEnergyCorrectionShift: " << TauEnergyCorrectionShift;
