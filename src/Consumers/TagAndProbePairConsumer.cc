...
std::string TagAndProbeMuonPairConsumer::GetConsumerId() const
{
	return "TagAndProbeMuonPairConsumer";
}

void TagAndProbeMuonPairConsumer::Init(setting_type const& settings, metadata_type& metadata) {
...
void TagAndProbeMuonPairConsumer::ProcessFilteredEvent(event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata )
{
...
void TagAndProbeMuonPairConsumer::Finish(setting_type const& settings, metadata_type const& metadata)
{
	RootFileHelper::SafeCd(settings.GetRootOutFile(), settings.GetRootFileFolder());
...
bool TagAndProbeMuonPairConsumer::IsMediumMuon2016ShortTerm(KMuon* muon) const
{
...
bool TagAndProbeMuonPairConsumer::IsMediumMuon2016(KMuon* muon) const
{
        bool goodGlob = muon->isGlobalMuon()
                                        && muon->normalizedChiSquare < 3
                                        && muon->chiSquareLocalPos < 12
...
std::string TagAndProbeElectronPairConsumer::GetConsumerId() const
{
	return "TagAndProbeElectronPairConsumer";
}

void TagAndProbeElectronPairConsumer::Init(setting_type const& settings, metadata_type& metadata) {
	ConsumerBase<HttTypes>::Init(settings, metadata);
...
void TagAndProbeElectronPairConsumer::ProcessFilteredEvent(event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata )
{
	ConsumerBase<HttTypes>::ProcessFilteredEvent(event, product, settings, metadata);
...
void TagAndProbeElectronPairConsumer::Finish(setting_type const& settings, metadata_type const& metadata)
{
	RootFileHelper::SafeCd(settings.GetRootOutFile(), settings.GetRootFileFolder());
	m_tree->Write(m_tree->GetName());
}

bool TagAndProbeElectronPairConsumer::IsMVABased(KElectron* electron, event_type const& event, const std::string &idName) const
{
...
std::string TagAndProbeGenTauConsumer::GetConsumerId() const
{
	return "TagAndProbeGenTauConsumer";
}

void TagAndProbeGenTauConsumer::Init(setting_type const& settings, metadata_type& metadata) {
	ConsumerBase<HttTypes>::Init(settings, metadata);
	//electronIDName = settings.GetElectronIDName();
	//electronMvaIDCutEB1 = settings.GetElectronMvaIDCutEB1();
	//electronMvaIDCutEB2 = settings.GetElectronMvaIDCutEB2();
...
void TagAndProbeGenTauConsumer::ProcessFilteredEvent(event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata )
{
	ConsumerBase<HttTypes>::ProcessFilteredEvent(event, product, settings, metadata);
...
void TagAndProbeGenTauConsumer::Finish(setting_type const& settings, metadata_type const& metadata)
{
	RootFileHelper::SafeCd(settings.GetRootOutFile(), settings.GetRootFileFolder());
	m_tree->Write(m_tree->GetName());
}

bool TagAndProbeGenTauConsumer::IsTauIDRecommendation13TeV(KTau* tau, event_type const& event, bool const& oldTauDMs, bool const& isAOD) const
{
...
std::string TagAndProbeGenMuonConsumer::GetConsumerId() const
{
	return "TagAndProbeGenMuonConsumer";
}

void TagAndProbeGenMuonConsumer::Init(setting_type const& settings, metadata_type& metadata) {
	ConsumerBase<HttTypes>::Init(settings, metadata);
...
void TagAndProbeGenMuonConsumer::ProcessFilteredEvent(event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata )
{
	ConsumerBase<HttTypes>::ProcessFilteredEvent(event, product, settings, metadata);
...
void TagAndProbeGenMuonConsumer::Finish(setting_type const& settings, metadata_type const& metadata)
{
	RootFileHelper::SafeCd(settings.GetRootOutFile(), settings.GetRootFileFolder());
	m_tree->Write(m_tree->GetName());
}
...
// https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2#Short_Term_Medium_Muon_Definitio
bool TagAndProbeGenMuonConsumer::IsMediumMuon2016ShortTerm(KMuon* muon) const
{
        bool goodGlob = muon->isGlobalMuon()
                                        && muon->normalizedChiSquare < 3
                                        && muon->chiSquareLocalPos < 12
...
bool TagAndProbeGenMuonConsumer::IsMediumMuon2016(KMuon* muon) const
{
        bool goodGlob = muon->isGlobalMuon()
                                        && muon->normalizedChiSquare < 3
                                        && muon->chiSquareLocalPos < 12
...
std::string TagAndProbeGenElectronConsumer::GetConsumerId() const
{
	return "TagAndProbeGenElectronConsumer";
}

void TagAndProbeGenElectronConsumer::Init(setting_type const& settings, metadata_type& metadata) {
	ConsumerBase<HttTypes>::Init(settings, metadata);
	electronIDName = settings.GetElectronIDName();
	electronMvaIDCutEB1 = settings.GetElectronMvaIDCutEB1();
	electronMvaIDCutEB2 = settings.GetElectronMvaIDCutEB2();
...
void TagAndProbeGenElectronConsumer::ProcessFilteredEvent(event_type const& event, product_type const& product, setting_type const& settings, metadata_type const& metadata )
{
	ConsumerBase<HttTypes>::ProcessFilteredEvent(event, product, settings, metadata);
...
void TagAndProbeGenElectronConsumer::Finish(setting_type const& settings, metadata_type const& metadata)
{
	RootFileHelper::SafeCd(settings.GetRootOutFile(), settings.GetRootFileFolder());
	m_tree->Write(m_tree->GetName());
}

bool TagAndProbeGenElectronConsumer::IsMVABased(KElectron* electron, event_type const& event, const std::string &idName) const
{
	bool validElectron = true;
	validElectron = validElectron && (electron->track.nInnerHits <= 1);
	validElectron = validElectron && (! (electron->electronType & (1 << KElectronType::hasConversionMatch)));
