...
{
	std::map<std::string, std::map<std::string, KLV*> > detailedTriggerMatchedLepton1 = SafeMap::GetWithDefault(*detailedTriggerMatchedLeptons, &(*first), std::map<std::string, std::map<std::string, KLV*> >());
	
	std::vector<std::string> hltPaths1;
	for (std::map<std::string, std::map<std::string, KLV*> >::iterator it = detailedTriggerMatchedLepton1.begin(); it != detailedTriggerMatchedLepton1.end(); ++it)
	{
		hltPaths1.push_back(it->first);
	}
	
	std::map<std::string, std::map<std::string, KLV*> > detailedTriggerMatchedLepton2 = SafeMap::GetWithDefault(*detailedTriggerMatchedLeptons, &(*second), std::map<std::string, std::map<std::string, KLV*> >());
	
	std::vector<std::string> hltPaths2;
	for (std::map<std::string, std::map<std::string, KLV*> >::iterator it = detailedTriggerMatchedLepton2.begin(); it != detailedTriggerMatchedLepton2.end(); ++it)
	{
