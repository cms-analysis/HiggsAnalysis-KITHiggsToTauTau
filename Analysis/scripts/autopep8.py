...
class FixPEP8(object):
    ...
    def _execute_pep8(self, targetfile, ignorelist):
        """execute pep8 via subprocess.Popen."""
        paths = os.environ['PATH'].split(':')
        paths.reverse()
        for path in paths:
            if os.path.exists(path + '/' + pep8bin):
                cmd = "%s/%s -r --ignore=%s %s" % (path, pep8bin, ignorelist, targetfile)
    ...
    def _fix_source(self):
        for result in self.results:
            ...
            if hasattr(self, fixed_methodname):
                ...
                if fixed_methodname[4:] not in ['w292']:
    ...
    def fix(self):
        pep8result = self._execute_pep8(self.filename, ignorelist)
