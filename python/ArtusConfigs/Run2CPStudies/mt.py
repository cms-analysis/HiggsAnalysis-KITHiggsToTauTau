...
class mt_ArtusConfig(dict):
    ...
    def build_config(self, nickname, *args, **kwargs):
        ...
        # I believe "Run2016|Spring16|Summer16|Embedding(2016|MC)" is everything else but for safety i did it here, 2017 not included yet
        else:
            self["DiTauPairLepton1LowerPtCuts"] = [
                "HLT_IsoMu24_v:25.0",
                "HLT_IsoTkMu24_v:25.0"
        ...
        if re.search("(Fall15MiniAODv2|Run2015D|Embedding2015)", nickname):
            ...
            self["TriggerEfficiencyData"] = [
                "0:$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/root/triggerWeights/triggerEfficiency_Run2016_Muon_Mu22OR_eta2p1_eff.root"]
            self["TriggerEfficiencyMc"] = [
                "0:$CMSSW_BASE/src/HiggsAnalysis/KITHiggsToTauTau/data/root/triggerWeights/triggerEfficiency_MC_Muon_Mu22OR_eta2p1_eff.root"]
            ...
            if re.search("Run2016", nickname):
                ...
                if re.search("(LFV).*(?=(Spring16|Summer16))", nickname):
                    ...
                    if re.search("(DY.?JetsToLL).*(?=(Spring16|Summer16))", nickname):
                        ...
                        #self["Processors"] += ["producer:MVATestMethodsProducer"]
                        self["Processors"] += ["producer:GenMatchedTauCPProducer"]
                        # I put this on in to create the same config as json but i dont think it is needed
                        self["Processors"] += ["producer:GenMatchedPolarisationQuantitiesProducer"]
                        #self["Processors"] += ["producer:TauPolarisationTmvaReader"]
