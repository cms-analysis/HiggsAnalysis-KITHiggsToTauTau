...
class processors_ordered(dict):
    ...
    def create_sorting_keydict(self, *args, **kwargs):
        ...
        if kwargs.get("channel", None) == "EM":
            ...
            self["producer:RecoElectronGenParticleMatchingProducer"] = 27
        ...
        # Producer of the devil, I hope he approves this if I go to hell
        self["producer:PolarisationQuantitiesSimpleFitProducer"] = 666
    ...
    def order_processors(self, processorlist, *args, **kwargs):
        ...
        while True:
            try:
                ...
                log.warning(
                    "keyerror while sorting processors, missing processor='%s'" % str(e.args[0]))
                log.warning("appending it at the bottom with number '%s', is this intended?" % str(
                    processornumber))
                self[e.args[0]] = processornumber
                processornumber += 1
                if processornumber > 1005:
                    log.error(
                        "missing more then 5 keys in ordering dict, check HiggsAnalysis/KITHiggsToTauTau/python/ArtusConfigs/processorOrdering.py")
                    break
            else:
                log.error(
                    "error in sorting processors, HiggsAnalysis/KITHiggsToTauTau/python/ArtusConfigs/processorOrdering.py")
                break
