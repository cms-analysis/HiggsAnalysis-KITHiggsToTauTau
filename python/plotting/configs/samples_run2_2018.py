...
class Samples(samples.Samples):
	...
	def wj_stitchingweight(self, use_ext_sample=True):
		# return "(((npartons == 0 || npartons >= 5)*(1.37088105e-03)) + ((npartons == 1)*(1.54354730e-04)) + ((npartons == 2)*(3.62872916e-04)) + ((npartons == 3)*(5.61528581e-05)) + ((npartons == 4)*(5.36308423e-05)))/(numberGeneratedEventsWeight*crossSectionPerEventWeight)"
		if self.legacy:
			return "(((npartons == 0 || npartons >= 5)*(1.20445099e-03)) + ((npartons == 1)*(3.02742512e-04)) + ((npartons == 2)*(3.50068766e-04)) + ((npartons == 3)*(7.45809024e-05)) + ((npartons == 4)*(5.96329744e-05)))/(numberGeneratedEventsWeight*crossSectionPerEventWeight)"
		else:
			if use_ext_sample:
				return "(((npartons == 0 || npartons >= 5)*(8.602609716e-04)) + ((npartons == 1)*(1.446850624e-04)) + ((npartons == 2)*(3.219452396e-04)) + ((npartons == 3)*(5.482001534e-05)) + ((npartons == 4)*(5.241373841e-05)))/(numberGeneratedEventsWeight*crossSectionPerEventWeight)"
			else:
				return "(((npartons == 0 || npartons >= 5)*(2.28776768e-03)) + ((npartons == 1)*(1.61649265e-04)) + ((npartons == 2)*(4.20029057e-04)) + ((npartons == 3)*(5.70900669e-05)) + ((npartons == 4)*(5.44851148e-05)))/(numberGeneratedEventsWeight*crossSectionPerEventWeight)"
	...
	def files_data(self, channel):
		...
		query_promptreco = {}
		query_reminiaod = {}
		expect_n_results_rereco = 3 # adjust in if-statements if different depending on channel
		...
		query_rereco["campaign"] = "Run2018(A|B|C)"
		query_rereco["scenario"] = "17Sep2018v.*"
		query_promptreco["data"] = True
		query_promptreco["campaign"] = "Run2018D"
		...
		return " ".join([rereco_files, promptreco_files])
	...
	def files_ztt(self, channel, embedding=False):
		if embedding:
			if channel=='mt':
				return self.artus_file_names({"process" : "Embedding2018.*" , "campaign" : "MuTauFinalState" , "scenario" : "inputDoubleMu_102X_miniAODv1"}, 4)
			elif channel=='et':
				return self.artus_file_names({"process" : "Embedding2018.*" , "campaign" : "ElTauFinalState" , "scenario" : "inputDoubleMu_102X_miniAODv1"}, 4)
			elif channel=='tt':
				return self.artus_file_names({"process" : "Embedding2018.*" , "campaign" : "TauTauFinalState" , "scenario" : "inputDoubleMu_102X_miniAODv1"}, 4)
			elif channel=='em':
				return self.artus_file_names({"process" : "Embedding2018.*" , "campaign" : "ElMuFinalState" , "scenario" : "inputDoubleMu_102X_miniAODv1"}, 4)
		else:
			artus_files = self.artus_file_names({"process" : "(DYJetsToLL_M10to50|DY(|1|2|3|4)JetsToLL_M50)", "data": False, "campaign" : self.mc_campaign, "generator" : "madgraph\-pythia8"}, 6)
			return artus_files
	...
	def files_ewkwm(self, channel):
		ewkw_query = { "data" : False,
						...
						"process" : "EWKWMinus2Jets_WToLNu_M50",
						"scenario" : "102X"}
	...
	def files_ewkwp(self, channel):
		ewkw_query = { "data" : False,
						"campaign" : self.mc_campaign,
						"generator" : "madgraph-pythia8",
						"process" : "EWKWPlus2Jets_WToLNu_M50",
						"scenario" : "102X"}
		artus_files = self.artus_file_names(ewkw_query, 1)
		return artus_files
	...
	def files_wj(self, channel, use_ext_sample=True):
		artus_files = self.artus_file_names({"process" : "W(|1|2|3|4)JetsToLNu", "data" : False, "campaign" : self.mc_campaign, "generator" : "madgraph-pythia8"}, 5)
		return artus_files

	############ DIBOSON FILES ###############
	def files_wwtolnuqq(self, channel):
		artus_files = self.artus_file_names({ "process" : "WWToLNuQQ","data" : False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)
		return artus_files

	def files_wwto4q(self, channel):
		# artus_files = self.artus_file_names({ "process" : "WWTo4Q", "data" : False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 0)
		# return artus_files
		return ""

	def files_wwto2l2nu(self, channel):
		artus_files = self.artus_file_names({ "process" : "WWTo2L2Nu", "data" : False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)
		return artus_files
	...
	def files_wzto3lnu(self, channel):
		artus_files = self.artus_file_names({ "process" : "WZTo3LNu", "data" : False, "campaign" : self.mc_campaign, "generator" : "amcatnlo-pythia8"}, 2)
		return artus_files

	def files_wzto2l2q(self, channel):
		artus_files = self.artus_file_names({ "process" : "WZTo2L2Q", "data" : False, "campaign" : self.mc_campaign, "generator" : "amcatnlo-pythia8"}, 1)
		return artus_files

	def files_wzto1l1nu2q(self, channel):
		artus_files = self.artus_file_names({ "process" : "WZTo1L1Nu2Q", "data" : False, "campaign" : self.mc_campaign, "generator" : "amcatnlo-pythia8"}, 1)
		return artus_files

	def files_zzto2l2nu(self, channel):
		artus_files = self.artus_file_names({ "process" : "ZZTo2L2Nu", "data" : False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 2)
		return artus_files
	...
	def files_zzto2l2q(self, channel):
		artus_files = self.artus_file_names({ "process" : "ZZTo2L2Q", "data" : False, "campaign" : self.mc_campaign, "generator" : "amcatnlo-pythia8"}, 1)
		return artus_files

	def files_ww(self, channel):
		artus_files = self.artus_file_names({ "process" : "WW", "data" : False, "campaign" : self.mc_campaign, "generator" : "pythia8"}, 1)
		return artus_files

	def files_wz(self, channel):
		artus_files = self.artus_file_names({ "process" : "WZ", "data" : False, "campaign" : self.mc_campaign, "generator" : "pythia8"}, 1)
		return artus_files

	def files_zz(self, channel):
		artus_files = self.artus_file_names({ "process" : "ZZ", "data" : False, "campaign" : self.mc_campaign, "generator" : "pythia8"}, 1)
		return artus_files

	def files_singletop(self, channel):
		artus_files = self.artus_file_names({
			"process" : "STtW(|anti)top5finclusiveDecaysTuneCP5",
			"data" : False,
			"campaign" : self.mc_campaign,
			"generator" : "powheg-pythia8"},
			2)
		artus_files = artus_files + " " + self.artus_file_names({
			"process" : "STt\-channel(|anti)top4fInclusiveDecaysTuneCP5",
			"data" : False,
			"campaign" : self.mc_campaign,
			"generator" : "powheg-pythia8"},
			2)
	...
	def files_diboson(self, channel):
		artus_files = " ".join[
			self.files_wwtolnuqq(channel),
			self.files_wwto4q(channel),
			self.files_wwto2l2nu(channel),
			self.files_wzto1l3nu(channel),
			self.files_wzto3lnu(channel),
			self.files_wzto2l2q(channel),
			self.files_wzto1l1nu2q(channel),
			self.files_zzto2l2nu(channel),
			self.files_zzto4l(channel),
			self.files_singletop(channel)]
		return artus_files
	...
	def vvt(self, config, channel, category, weight, nick_suffix, lumi=default_lumi, exclude_cuts=None, cut_type="baseline", fakefactor_method=False, **kwargs):
		...
		if channel in ["mt", "et", "tt"]:
			diboson_weight = "*".join([
				...
				weight,
				"eventWeight",
				Samples.ttt_genmatch(channel),
				self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type),
				self.embedding_ttbarveto_weight(channel),
				self.nojetsfakefactor_weight(channel, fakefactor_method=fakefactor_method)
			])
			nick = "vvt"

			if kwargs.get("inclusive_diboson_samples", False):
				add_input(
						input_file=self.files_ww(channel),
						weight=diboson_weight,
						nick=nick
				)
				add_input(
				...
				add_input(
						...
						weight=diboson_weight+"*"+self.wwtolnuqq_stitchingweight(),
						nick=nick
				)
				add_input(
				...
				add_input(
						input_file=self.files_wwto2l2nu(channel),
						weight=diboson_weight,
						nick=nick
				)
				add_input(
				...
				add_input(
						input_file=self.files_wzto2l2q(channel),
						weight=diboson_weight,
						nick=nick
				)
				add_input(
						input_file=self.files_wzto1l1nu2q(channel),
						weight=diboson_weight,
						nick=nick
				)
				add_input(
				...
				add_input(
						...
						weight=diboson_weight+"*"+self.zzto4l_stitchingweight(),
						nick=nick
				)
			add_input(
	...
	def files_qqh(self, channel, mass=125, **kwargs):
		...
		if state == "initialState":
			...
			return self.artus_file_names({"process" : "VBFHToTauTauUncorrelatedDecay_Filtered_M125", "data": False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)
		else:
			return self.artus_file_names({"process" : "VBFHToTauTau_M"+str(mass), "data": False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)

	def files_wh(self, channel, mass=125, **kwargs):
		return self.artus_file_names({"process" : "W(plus|minus)HToTauTauUncorrelatedDecay_Filtered_M125", "data": False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 2)

	def files_zh(self, channel, mass=125, **kwargs):
		return self.artus_file_names({"process" : "ZHToTauTauUncorrelatedDecay_Filtered_M125", "data": False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)
	...
	def files_ggh(self, channel, mass=125, **kwargs):
		...
		if state == "initialState":
			...
			return self.artus_file_names({"process" : "GluGluHToTauTauUncorrelatedDecay_Filtered_M125", "data": False, "campaign" : self.mc_campaign, "generator" : "powheg-pythia8"}, 1)
	...
	def ff(self, config, channel, category, weight, nick_suffix, lumi=default_lumi, exclude_cuts=None, cut_type="baseline", fake_factor_name_1="fakefactorWeight_comb_inclusive_1", fake_factor_name_2="fakefactorWeight_comb_inclusive_2", fakefactor_method=True, **kwargs):
		...
		if channel == "tt":
			if self.legacy:
				if cpfinalstate:
					ff_weight_1 = "(" + (proxy_fakefactor_weight_1 if proxy_fakefactors else fake_factor_name_1) + ")"
		...
		if channel in ["mt", "et", "tt"]:
			for sub_channel in sub_channels:
				...
				add_input(
					input_file=self.files_ztt(channel, embedding=self.embedding),
					weight=mc_weight+"*"+self.ztt_genmatch(channel, embedding=self.embedding)+"*"+self.get_weights_ztt(channel=channel,cut_type=cut_type,weight=weight_ff+"*"+weight_ff_iso+"*"+weight_tau_id,embedding=self.embedding)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts+exclude_cuts_ff, cut_type=cut_type, embedding=self.embedding)+"*"+self.decay_mode_reweight(channel, cut_type),
					scale_factor = 1.0 if self.embedding else lumi,
	...
	def wfakes(self, config, channel, category, weight, nick_suffix, lumi=default_lumi, exclude_cuts=None, cut_type="baseline", **kwargs):
		...
		wfakes_weight = weight

		if exclude_cuts is None:
			exclude_cuts = []

		if channel == "tt":
			if self.legacy and cpfinalstate:
				wfakes_weight = "*".join([wfakes_weight, "((gen_match_1!=6) && (gen_match_2==6))"])
		...
		if channel in ["tt"]:
			...
			add_input(
				input_file=self.files_ztt(channel, embedding=self.embedding),
				weight=mc_weight+"*"+self.ztt_genmatch(channel, embedding=self.embedding)+"*"+self.get_weights_ztt(channel=channel,cut_type=cut_type_emb,weight=wfakes_weight,embedding=self.embedding)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type_emb, embedding=self.embedding)+"*"+self.decay_mode_reweight(channel, cut_type_emb),
				scale_factor = 1.0 if self.embedding else lumi,
				nick="wfakes"
			)
			"""
			add_input(
				input_file=self.files_zll(channel),
				weight=self.get_weights_zll(channel=channel,cut_type=cut_type,weight=weight_ff)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type)+"*zPtReweightWeight*(gen_match_2 < 6)",
				nick="wfakes"
			)"""

			add_input(
				input_file=self.files_zll(channel),
				weight=mc_weight+"*eventWeight*"+wfakes_weight+"*(zPtReweightWeight)*"+self.zll_stitchingweight()+"*"+Samples.zl_genmatch(channel)+"*"+self._cut_string(channel, exclude_cuts= exclude_cuts, cut_type=cut_type)+"*"+self.zll_zl_shape_weight(channel, cut_type)+"*"+zmm_cr_factor+"*"+self.em_triggerweight_dz_filter(channel, cut_type=cut_type),
				nick="wfakes"
				)
			add_input(
				input_file=self.files_ttj(channel),
				weight=mc_weight+"*eventWeight*"+wfakes_weight+"*"+self.embedding_ttbarveto_weight(channel)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type),
				nick="wfakes"
			)
			if kwargs.get("inclusive_diboson_samples", False):
				add_input(
				...
				add_input(
					input_file=self.files_wwtolnuqq(channel),
					weight=mc_weight+"*eventWeight*"+wfakes_weight+"*"+self.embedding_ttbarveto_weight(channel)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type)+"*"+self.wwtolnuqq_stitchingweight(),
					nick="wfakes"
				)
				# add_input(
				# 	input_file=self.files_wwto4q(channel),
				...
				add_input(
					input_file=self.files_zzto4l(channel),
					weight=mc_weight+"*eventWeight*"+wfakes_weight+"*"+self.embedding_ttbarveto_weight(channel)+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type)+"*"+self.zzto4l_stitchingweight(),
					nick="wfakes"
				)
			add_input(
				input_file=self.files_singletop(channel),
				weight=mc_weight+"*eventWeight*"+wfakes_weight+"*"+self._cut_string(channel, exclude_cuts=exclude_cuts, cut_type=cut_type),
				nick="wfakes"
			)
		else:
			log.error("Sample config (W Fakes) currently not implemented for channel \"%s\"! Sample config (W Fakes) is only needed for the fake factor method in the tt channel for the CP analysis." % channel)

		Samples._add_plot(config, "bkg", "HIST", "F", "wfakes", nick_suffix)
		return config
