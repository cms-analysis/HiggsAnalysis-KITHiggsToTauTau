	...
	def wj(self, config, channel, weight, nick_suffix, lumi=19712.0, exclude_cuts=None, **kwargs):
		...
		if channel == "tt":
			...
			Samples._add_input(
					...
					channel+"_dirIso_z_tauEs/ntuple",
					1.0,
					weight+"*eventWeight*" + Samples.cut_string(channel, exclude_cuts=exclude_cuts+["mt"]) + "*(lep1MetMt>70.0)",
	...
	def qcd(self, config, channel, weight, nick_suffix, lumi=19712.0, exclude_cuts=None, **kwargs):
		...
		if channel == "tt":
			...
			Samples._add_input(
					...
					channel+"_dirIso_z_tauEs/ntuple",
					1.0,
					weight+"*eventWeight*" + Samples.cut_string(channel, exclude_cuts=exclude_cuts+["os", "mt"]) + "*((q_1*q_2)>0.0)*(lep1MetMt>70.0)",
