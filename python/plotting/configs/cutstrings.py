...
class CutStringsDict:
	...
	def baseline(channel, cut_type):
		...
		if channel == "mm":
			...
			cuts["trigger_threshold"] = "(pt_1 > 20.0)*(pt_2 > 20.0)"#"(pt_1 > 25.0 && pt_2 > 20.0)*(pt_1 > pt_2) + (pt_1 > 20.0 && pt_2 > 25.0)*(pt_1 <= pt_2)" if "2016" in cut_type else "(1.0)"
			cuts["extra_lepton_veto"] = "(extraelec_veto < 0.5)*(extramuon_veto < 0.5)"
			...
			cuts["m_vis"] = "(m_vis > 12)"#*(m_vis < 80)"
			#cuts["njets"] = "(njets == 0)"
			#cuts["nbtag"] = "(nbtag > 0)"
			cuts["bveto"] = "(bpt_1 < 20)"
			#cuts["dcosphi"] = "(dcosphi > 0.0)"
			#cuts["deta"] = "(deta < 3.8)*(deta > -3.8)"
			#cuts["pzeta"] = "(pZetaMissVis < -40.0)"
			cuts["iso_1"] = "(iso_1 < 0.15)"
