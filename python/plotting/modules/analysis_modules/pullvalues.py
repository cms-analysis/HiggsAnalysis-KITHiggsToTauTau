...
class ComputePullValues(analysisbase.AnalysisBase):
	...
	def modify_argument_parser(self, parser, args):
		...
		self.pullvalues_options.add_argument("--fit-s-only", type="bool", default=False,
				help="Signal-only fit. [Default: %(default)s]")
	...
	def prepare_args(self, parser, plotData):
		...
		if plotData.plotdict["fit_s_only"]:
			self._plotdict_keys.remove("fit_b_nick")

		self.prepare_list_args(plotData, self._plotdict_keys)

		for key in self._plotdict_keys:
			assert (isinstance(plotData.plotdict["root_objects"].get(plotData.plotdict[key][0]), ROOT.RooFitResult) or
				isinstance(plotData.plotdict["root_objects"].get(plotData.plotdict[key][0]), ROOT.RooArgSet))
			plotData.plotdict['nicks_blacklist'].append(plotData.plotdict[key][0])
	...
	def run(self, plotData=None):
		...
		prefit = plotData.plotdict["nuisances_prefit_nick"][0]
		fit_s = plotData.plotdict["fit_s_nick"][0]
		fit_b = plotData.plotdict["fit_b_nick"][0]

		#get the fitted parameter
		fpf_s = plotData.plotdict["root_objects"].get(fit_s).floatParsFinal()
		if not plotData.plotdict["fit_s_only"]:
			fpf_b = plotData.plotdict["root_objects"].get(fit_b).floatParsFinal()

		pulls = []
		nuis_p_i = 0

		hist_fit_b  = ROOT.TH1F("prefit_fit_b","B-only fit Nuisances",plotData.plotdict["root_objects"].get(prefit).getSize(),0,plotData.plotdict["root_objects"].get(prefit).getSize())
		hist_fit_s  = ROOT.TH1F("prefit_fit_s","S+B fit Nuisances",plotData.plotdict["root_objects"].get(prefit).getSize(),0,plotData.plotdict["root_objects"].get(prefit).getSize())
		hist_prefit = ROOT.TH1F("prefit_nuisancs","Prefit Nuisances",plotData.plotdict["root_objects"].get(prefit).getSize(),0,plotData.plotdict["root_objects"].get(prefit).getSize())
		...
		for i in range(fpf_s.getSize()):
			...
			nuisance_list = [('s',nuis_s)] if plotData.plotdict["fit_s_only"] else [('b', nuis_b), ('s',nuis_s)]
			for fit_name, nuis_x in nuisance_list:
				if nuis_x == None:
					row += [ " n/a " ]
				else:
		...
		if plotData.plotdict["fit_s_only"]:
	        	fmtstring = "%-40s     %15s    %30s  %15s"
			log.debug(fmtstring % ('name', 'pre fit', 's+b fit', 'rho'))
		else:
		...
		for n in names:
			...
			if plotData.plotdict["fit_s_only"]:
				log.debug(fmtstring % (n, v[0], v[1], v[2]))
			else:
