...
class CPStudiesDatacards(datacards.Datacards):
	def __init__(self, cp_mixing_angles_over_pi_half=[], add_data=True, cb=None):
		...
		if cb is None:
			...
			# efficiencies
			self.cb.cp().channel(["mt"]).process(["ZTT", "ZLL"]).AddSyst(self.cb, *self.muon_efficieny_syst_args)
			self.cb.cp().channel(["mt"]).signals().AddSyst(self.cb, *self.muon_efficieny_syst_args)

			self.cb.cp().channel(["mt"]).process(["ZTT"]).AddSyst(self.cb, *self.tau_efficieny_syst_args)
			self.cb.cp().channel(["mt"]).signals().AddSyst(self.cb, *self.tau_efficieny_syst_args)
			self.cb.cp().channel(["mt"]).process(["ZTT"]).AddSyst(self.cb, *self.tau_efficieny_corr_syst_args)
			...
			# efficiencies
			self.cb.cp().channel(["et"]).process(["ZTT", "ZLL"]).AddSyst(self.cb, *self.electron_efficieny_syst_args)
			self.cb.cp().channel(["et"]).signals().AddSyst(self.cb, *self.electron_efficieny_syst_args)

			self.cb.cp().channel(["et"]).process(["ZTT"]).AddSyst(self.cb, *self.tau_efficieny_syst_args)
			...
			# efficiencies
			self.cb.cp().channel(["em"]).process(["ZTT", "ZLL"]).AddSyst(self.cb, *self.electron_efficieny_syst_args)
			self.cb.cp().channel(["em"]).signals().AddSyst(self.cb, *self.electron_efficieny_syst_args)

			self.cb.cp().channel(["em"]).process(["ZTT", "ZLL"]).AddSyst(self.cb, *self.muon_efficieny_syst_args)
			...
			# efficiencies
			self.cb.cp().channel(["tt"]).process(["ZTT"]).AddSyst(self.cb, *self.tau_efficieny_syst_args)
			...
			self.cb.cp().process(["ZTT", "ZLL"]).AddSyst(self.cb, *self.lumi_syst_args)
