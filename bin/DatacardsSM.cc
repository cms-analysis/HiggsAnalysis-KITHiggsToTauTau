...
int main() {
	...
	string auxiliaries = string(getenv("CMSSW_BASE")) + "/src/HiggsAnalysis/KITHiggsToTauTau/auxiliaries/combine/";
	...
	for (string era : {"7TeV", "8TeV"}) {
		for (string chn : chns) {
			...
			string file = aux_shapes + chn + "/htt_" + chn + ".inputs-sm-" + era + ".root";
	...
	cb_et.cp().era({"7TeV"}).bin_id({6}).process({"ZL", "ZJ", "W", "ZTT"}).MergeBinErrors(0.1, 0.5);
	cb_et.cp().era({"8TeV"}).bin_id({7}).process({"ZL", "ZJ", "W", "ZTT"}).MergeBinErrors(0.1, 0.5);
	...
	cb_mt.cp().era({"7TeV"}).bin_id({6}).process({"W", "ZTT"}).MergeBinErrors(0.1, 0.5);
	cb_mt.cp().era({"8TeV"}).bin_id({5, 6}).process({"W"}).MergeBinErrors(0.1, 0.5);
	...
	cb_em.cp().era({"8TeV"}).bin_id({5}).process({"Fakes", "EWK", "Ztt"}).MergeBinErrors(0.1, 0.5);
	...
	cb_tt.cp().bin_id({0, 1, 2}).era({"8TeV"}).process({"ZTT", "QCD"}).MergeBinErrors(0.1, 0.5);

	cout << ">> Generating bbb uncertainties...\n";
	cb_mt.cp().bin_id({0, 1, 2, 3, 4}).process({"W", "QCD"}).AddBinByBin(0.1, true, &cb);
	cb_mt.cp().era({"7TeV"}).bin_id({5}).process({"W"}).AddBinByBin(0.1, true, &cb);
	cb_mt.cp().era({"7TeV"}).bin_id({6}).process({"W", "ZTT"}).AddBinByBin(0.1, true, &cb);
	cb_mt.cp().era({"8TeV"}).bin_id({5, 6}).process({"W"}).AddBinByBin(0.1, true, &cb);
	cb_mt.cp().era({"8TeV"}).bin_id({7}).process({"W", "ZTT"}).AddBinByBin(0.1, true, &cb);

	cb_et.cp().bin_id({1, 2}).process({"ZL", "ZJ", "QCD", "W"}).AddBinByBin(0.1, true, &cb);
	cb_et.cp().bin_id({3, 5}).process({"W"}).AddBinByBin(0.1, true, &cb);
	cb_et.cp().era({"7TeV"}).bin_id({6}).process({"ZL", "ZJ", "W", "ZTT"}).AddBinByBin(0.1, true, &cb);
	cb_et.cp().era({"8TeV"}).bin_id({7}).process({"ZL", "ZJ", "W", "ZTT"}).AddBinByBin(0.1, true, &cb);
	cb_et.cp().era({"8TeV"}).bin_id({6}).process({"ZL", "ZJ", "W"}).AddBinByBin(0.1, true, &cb);

	cb_em.cp().bin_id({1, 3}).process({"Fakes"}).AddBinByBin(0.1, true, &cb);
	cb_em.cp().era({"7TeV"}).bin_id({4}).process({"Fakes", "EWK", "Ztt"}).AddBinByBin(0.1, true, &cb);
	cb_em.cp().era({"8TeV"}).bin_id({5}).process({"Fakes", "EWK", "Ztt"}).AddBinByBin(0.1, true, &cb);
	cb_em.cp().era({"8TeV"}).bin_id({4}).process({"Fakes", "EWK"}).AddBinByBin(0.1, true, &cb);

	cb_ee_mm.cp().bin_id({1, 3, 4}).process({"ZTT", "ZEE", "ZMM", "TTJ"}).AddBinByBin(0.0, true, &cb);

	cb_tt.cp().bin_id({0, 1, 2}).era({"8TeV"}).process({"QCD", "ZTT"}).AddBinByBin(0.1, true, &cb);

	cout << ">> Setting standardised bin names...\n";
	ch::SetStandardBinNames(cb);
	VString droplist = ch::ParseFileLines(aux_pruning + "uncertainty-pruning-drop-131128-sm.txt");
