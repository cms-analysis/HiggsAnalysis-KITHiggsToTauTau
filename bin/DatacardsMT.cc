...
int main() {
	...
	string aux = string(getenv("CMSSW_BASE")) + "/src/HiggsAnalysis/KITHiggsToTauTau/auxiliaries/combine/";
	string aux_shapes = aux + "shapes/";
	...
	cb.cp().backgrounds().ExtractShapes(
			aux_shapes + "mt/htt_mt.inputs-sm-8TeV.root",
			"$BIN/$PROCESS",
